...
fluid_sample_import_sfont(fluid_sample_t* sample, SFSample* sfsample, fluid_defsfont_t* sfont)
{
  ...
  /* Sanitise a malformed SoundFont so it can't crash fluid_voice_optimize_sample
     (which scans data[loopstart..loopend]) or playback. Reject a sample only when
     its DATA range is unusable; when just the loop points are out of range, clamp
     them into the sample so it still plays (upstream FluidSynth does the same). The
     tested SC-55 SF2 has this exact defect on both Pizzicato Strings samples (GM
     program 45), which the DOOM soundtrack uses — rejecting them would silence it. */
  {
    unsigned int maxframe = sfont->samplesize / 2; /* sampledata is 16-bit */
    if (sample->start >= maxframe || sample->end > maxframe || sample->start >= sample->end) {
      FLUID_LOG(FLUID_WARN, "Ignoring sample %s: sample data out of range", sample->name);
      sample->valid = 0;
      return FLUID_OK;
    }
    if (sample->loopend > sample->end) {
      FLUID_LOG(FLUID_WARN, "Clamping out-of-range loop of sample %s", sample->name);
      sample->loopend = sample->end;
      if (sample->loopstart >= sample->loopend || sample->loopstart < sample->start) {
        sample->loopstart = sample->start;
      }
    }
  }
