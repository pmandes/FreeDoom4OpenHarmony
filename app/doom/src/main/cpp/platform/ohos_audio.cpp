...
std::atomic<bool> g_userPaused{false}; // lifecycle pause (Home/background) — takes priority over resume-from-interrupt
...
// Audio focus interrupts (call, assistant, another app).
// On FORCE the system pauses/ducks the stream itself — we keep a consistent state
// and resume on HINT_RESUME, provided the game is not paused from lifecycle.
