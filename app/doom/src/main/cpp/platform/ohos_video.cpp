...
// nearest-neighbor, so the compositor doesn't interpolate.
int g_dstW = kFbWidth;
int g_dstH = kFbHeight;

// For each destination column: the source column index (nearest). Recomputed
// only when the buffer width changes — removes the division from the per-pixel loop.
...
void OnSurfaceAvailable(NativeWindow *window, int width, int height)
{
    ...
    // Fall back to 320x200 (compositor scales) when the size isn't known yet.
    ...
    // so we stick with RGBA_8888 + an R<->B swap in the blit.
    OH_NativeWindow_NativeWindowHandleOpt(window, SET_FORMAT, NATIVEBUFFER_PIXEL_FMT_RGBA_8888);
    // Without CPU_WRITE in the usage flags the buffer also composites as black.
...
bool PresentFrame(const uint32_t *frame)
{
    ...
    if (handle != nullptr) {
        ...
        if (dst != nullptr) {
            ...
            // size, stride in bytes (can be > width*4). Convert 0xAARRGGBB (doomgeneric)
