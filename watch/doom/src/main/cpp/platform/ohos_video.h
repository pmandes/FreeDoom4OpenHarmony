...
// Pixels in doomgeneric convention: uint32_t 0xAARRGGBB (little-endian ⇒ bytes B,G,R,A).
namespace video {
...
// 320x200 frame is scaled nearest-neighbor, so the compositor does a 1:1 blit (pixel-perfect,
...
// Stops the rendering thread and detaches the NativeWindow. Safe to call multiple times.
void OnSurfaceLost();

// Copy the 320x200 frame (0xAARRGGBB) into the window buffer and present it.
// Returns false if the surface does not exist or presentation failed.
