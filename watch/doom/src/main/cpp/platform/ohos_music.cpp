...
// renders in MixInto via try_lock (on a song change — one beat of music silence).
...
boolean I_OHOS_InitMusic(void)
{
    ...
    return true; // a missing soundfont doesn't block the game — it runs without music
}
...
void MixInto(int32_t *accum, int32_t frameCount)
{
    ...
    if (!lock.owns_lock()) {
        return; // song change in progress — one beat without music instead of blocking
    }
